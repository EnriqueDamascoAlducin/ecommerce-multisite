...
export default function StorefrontCategory({
    ...
    function navigate(attrs?: Record<string, unknown>) {
        const payload = attrs && Object.keys(attrs).length > 0 ? { attrs } : {};
            ...
            case 'multiselect':
                return (
                    <div key={attr.id} className="space-y-1">
                        <p className="text-sm font-medium text-neutral-700 dark:text-neutral-300">{attr.name}</p>
    ...
    return (
        <>
            ...
            <div className="flex gap-8">
                {filterOptions.attributes.length > 0 && (
                    <aside className={`${showFilters ? 'block' : 'hidden'} w-full shrink-0 space-y-6 md:block md:w-60`}>
                        <div className="flex items-center justify-between">
                            <h2 className="text-sm font-semibold text-neutral-700 dark:text-neutral-300">Filtros</h2>
                            {hasActiveFilters() && (
                                <button
                                    type="button"
                        ...
                        <div className="space-y-5">{filterOptions.attributes.map(renderAttributeFilter)}</div>
                    </aside>
                )}
