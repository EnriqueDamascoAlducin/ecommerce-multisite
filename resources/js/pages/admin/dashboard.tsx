...
export default function AdminDashboard({
    ...
    const maxRevenue = Math.max(
        ...revenueByDay.map((day) => Number(day.revenue)),
        1,
    );

    const cards = [
        { label: 'Ingresos', value: formatPrice(summary.revenue) },
        { label: 'Ordenes pagadas', value: String(summary.paid_orders) },
        { label: 'Ordenes totales', value: String(summary.total_orders) },
        {
        ...
        <>
            ...
            <div className="mb-6 flex flex-wrap items-start justify-between gap-3">
                <div>
                    ...
                    <p className="mt-1 text-sm text-neutral-500 dark:text-neutral-400">
                        Resumen de ventas y rendimiento por periodo.
                    </p>
                </div>
            </div>
                        ...
                        <div className="flex h-40 items-end gap-1">
                            {revenueByDay.map((point) => (
                                <div
                                    ...
                                    style={{
                                        height: `${Math.max((Number(point.revenue) / maxRevenue) * 100, 2)}%`,
                                    }}
                                    title={`${point.day}: ${formatPrice(point.revenue)} (${point.orders} ordenes)`}
                                />
                            ))}
                        </div>
                        <div className="mt-2 flex justify-between text-xs text-neutral-400">
                            <span>{revenueByDay[0]?.day}</span>
                            <span>{revenueByDay.at(-1)?.day}</span>
                        </div>
                    </>
                )}
            </section>
            ...
            <div className="grid gap-6 lg:grid-cols-2">
                <section className="overflow-hidden rounded-lg border border-neutral-200 bg-white dark:border-neutral-800 dark:bg-neutral-900">
                    <h2 className="border-b border-neutral-200 px-4 py-3 text-sm font-medium text-neutral-500 dark:border-neutral-800">
